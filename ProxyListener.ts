...
function wheelEventHandler(event, sendFn, preventDefault) {
  if(preventDefault && event.preventDefault) event.preventDefault();
...
function filteredKeydownEventHandler(event, sendFn, preventDefault) {
  let {keyCode} = event;
  if (keys[keyCode]) {
    if(preventDefault && event.preventDefault && (keyCode < 110 || keyCode > 123)) event.preventDefault();
...
export class ProxyManager {
    ...
    makeProxy = (id, addTo?:any) => {    //addTo installs the desirable functions to the object you want     
